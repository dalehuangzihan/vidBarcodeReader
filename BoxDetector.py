...
while(cap.isOpened()):
    ...
    if isFrameAvail:
        ...
        dilationKernel = cv2.getStructuringElement(cv2.MORPH_RECT, ksize=(5, 5))
        dilated = cv2.dilate(thresh, dilationKernel, iterations=2)
        ...
        closed = cv2.morphologyEx(dilated, cv2.MORPH_CLOSE, closingKernel, iterations=2)
        cv2.imshow('closed', closed)

        erosionKernel = cv2.getStructuringElement(cv2.MORPH_RECT, ksize=(5,5))
        eroded = cv2.erode(closed, erosionKernel, iterations=2)
        #cv2.imshow('eroded', eroded)

        ## find contours in threshold image, then sort by area
        contours, hierarchy = cv2.findContours(dilated.copy(), cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE) # findContours (cv2) will modify the source frame, so copy.
        #contours = imutils.grab_contours(contours) #dont need this line, we know we're using cv2
        ...
        for i, contour in enumerate(contours):
            ...
            if hierarchy[0][i][2] != -1:
                ...
                box = np.intp(box)
                scale = 10**5
                if cv2.contourArea(box) < 4*scale and cv2.contourArea(box) >= 0.6*scale:
                    cv2.drawContours(frame, [box], 0, (0, 0, 255), 3)
                if cv2.contourArea(box) < 0.6*scale and cv2.contourArea(box) > 0.1*scale:
